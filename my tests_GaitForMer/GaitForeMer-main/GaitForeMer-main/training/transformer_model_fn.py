...
import seq2seq_model_fn as seq2seq_model_fn
...
import data.CustomGaitDataset as CustomGaitDataset
...
class POTRModelFn(seq2seq_model_fn.ModelFn):
  ...
  def init_model(self, pose_encoder_fn=None, pose_decoder_fn=None):
    ...
    # self._model = PoseTransformer.model_factory(
    # self._params,
    # pose_encoder_fn=SimpleEncoder(n_nodes=49, input_features=3, model_dim=self._params['model_dim']),
    # pose_decoder_fn=pose_decoder_fn
    # )
...
def dataset_factory(params, fold, model_prefix):
  ...
  elif params['dataset'] == 'custom_gait':
    return CustomGaitDataset.dataset_factory(params, fold)
  else:
